...
class Store(object):
    ...
    def find_by_url(cls, url):
        ...
        :return: a store if found, error if not (StorenotFoundException)
        """
        for i in range(0, len(url)+1):
            try:
                ...
                if i == len(url):
                    raise StoreErrors.StoreNotFoundError("The URL Prefix used to find the store did not give any results.")
                else:
                    continue
